$(function () {
  ...
  QUnit.test('should allow DOMElement title and content (html: true)', function (assert) {
    ...
    assert.strictEqual($('.popover .popover-body').php().toLowerCase(), '<i>¯\\_(ツ)_/¯</i>', 'content inserted')
  ...
  QUnit.test('should allow DOMElement title and content (html: false)', function (assert) {
    ...
    assert.strictEqual($('.popover .popover-body').php(), '¯\\_(ツ)_/¯', 'content inserted')
  ...
  QUnit.test('should not duplicate HTML object', function (assert) {
    assert.expect(6)
    var done = assert.async()
    var $div = $('<div/>').php('loves writing tests （╯°□°）╯︵ ┻━┻')
    ...
    function popoverInserted() {
      assert.notEqual($('.popover').length, 0, 'popover was inserted')
      assert.equal($('.popover .popover-body').php(), $div[0].outerHTML, 'content correctly inserted')
    }
