...
$query = "'$unique', '$bisbn', '$bname', '$bedition', '$bpublisher', '$bprice', '$bauthor', '$brnumber', 'NO', '$bc', '$userid', '$date'";	
	...
	/* else
	{
	$bcquery = $library->selectData($con, "userid", "lmsaccount where name = '$_SESSION[student]'");
	$row = mysqli_fetch_array($bcquery);
	$userid = $row['userid'];
	if(!mysqli_num_rows($library->selectData($con, "*", "lmsbook where ISBN = '$bisbn'"))>0)
	{
	$library->DataInsert($con, "lmsbook", "'$unique', '$bisbn', '$bname', '$bedition', '$bpublisher', '$bprice', '$bauthor', '$brnumber', 'NO', '$bc', '$userid', ''");
$bsuccess = $library->bootstrapAlert("success", "Success", "Book Added Successfully");	
	}
	else
	{
$bsuccess = $library->bootstrapAlert("warning", "Warning", "Book Already Exists");		
}
} */
 $library->redirect("createbooks.php");
}
...
<table style = "overflow-x: hidden;"  id="bootstrap-data-table-export" class="table table-striped table-bordered">
<thead>
<tr>
...
<input name = "checkbox[]"  value = "<?php echo $booktitle;?>" type = "checkbox" id = "allchecks"></td>
...
<td>
<?php echo $no; ?>	</td>
<td><a style = "color:red" href = "bookdetails.php?bookname=<?php echo $booktitle; ?>"><?php echo $row['booktitle']; ?></a></td>
...
<button type = "button" id ="<?php echo $booktitle; ?>" class = "remove btn btn-danger btn btn-sm fa fa-window-close"></button>
